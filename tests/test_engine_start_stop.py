...
def test_engine_start_stop_conflicts():
    app = build_app()
    with TestClient(app) as client:
        ...
        # start
        assert client.post("/engine/start").status_code == 200
        # starting again yields conflict
        resp = client.post("/engine/start")
        assert resp.status_code == 409
        # stop
        assert client.post("/engine/stop").status_code == 200
        # stopping again yields conflict
        resp = client.post("/engine/stop")
        assert resp.status_code == 409
